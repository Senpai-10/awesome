	...
	common_tag = function(c, source_c)
		...
		for _, t in pairs(c:tags()) do
			for _, t2 in pairs(source_c:tags()) do
				if t == t2 then
					cyclefocus.debug(
						'common_tag_filter: client shares tag "'
							.. cyclefocus.get_object_name(t)
							.. '" with "'
							.. cyclefocus.get_object_name(c)
							.. '"',
						2
					)
					return true
...
cyclefocus.callback_show_client = function(c, restore)
	if callback_show_client_lock then
		return
	end
...
cyclefocus.cycle = function(startdirection_or_args, args)
	...
	local cycle_filters = awful.util.table.join(args.cycle_filters or {}, cyclefocus.cycle_filters)
			...
			if nextc then
				...
				if cycle_filters then
					...
					for _k, filter in pairs(cycle_filters) do
						cyclefocus.debug(
							"Checking filter " .. _k .. "/" .. #cycle_filters .. ": " .. tostring(filter),
							4
						)
	...
	capi.keygrabber.run(function(mod, key, event)
		...
		-- Sort the offsets.
		local offsets = {}
		for n in pairs(dlist) do
			table.insert(offsets, n)
		end
		table.sort(offsets)
