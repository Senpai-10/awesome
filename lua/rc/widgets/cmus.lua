...
local cmus_widget_container = wibox.container.background()
cmus_widget_container.visible = false
...
local function hook_cmus() --{{{
	...
	if cmus_state == "playing" or cmus_state == "paused" then
		...
		local cmus_curtime_formated = math.floor(cmus_curtime / 60) .. ":" .. string.format("%02d", cmus_curtime % 60)
...
local cmus_widget = wibox.widget({
	{
		{
			widget = awful.widget.watch("cmus", 1, function(widget)
				...
				if cmus_run then
					cmus_widget_container.visible = true
...
cmus_widget_container:set_widget(cmus_widget)
cmus_widget_container:set_fg(beautiful.widget_fg)

M.widget = cmus_widget_container
